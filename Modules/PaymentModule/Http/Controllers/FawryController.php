...
class FawryController extends Controller
{
    ...
    public function payment(Request $request): View|Factory|JsonResponse|Application
    {
        ...
        // Log::info('Payer information: ', $payer);
        $paramList = array();
        $ORDER_ID = time();
        ...
        $paramList["CUSTOMER_NAME"] = $payer->name;
        ...
        return view('paymentmodule::fawry', compact('checkSum', 'paramList'));
    }
    public function response(Request $request)
    {
        $allSession = (session()->all());
        $data = $this->payment::where(['id' => $allSession['payment_id']])->first();
        if ($request->statusCode == 200) {
            if ($request->orderStatus == 'PAID') {
                $this->payment::where(['id' => $allSession['payment_id']])->update([
        ...
        $payment_data = $this->payment::where(['id' => $allSession['payment_id']])->first();
    ...
    public function fawrycallbackreq(Request $request){
        dd($request);
    }

    public function fawryWebhook(Request $request) {
        Log::info('Received Fawry webhook: ', $request->all());
    
        $validated = $request->validate([
            'order_number' => 'required|string',
        ]);

        Log::info('Validated Fawry webhook request: ', $validated);
    
        $order =  $this->payment::where(['transaction_id' => $request->order_number])->first();
        
        if (!$order) {
            Log::error('Order not found for merchantRefNumber: ' . $request->merchantRefNumber);
        ...
        if ($request->orderStatus == 'PAID') {
            ...
            Log::info('Order updated to PAID status: ' . $order->id);
            if (isset($order) && function_exists($order->success_hook)) {
                call_user_func($order->success_hook, $order);
            }
        } else {
            Log::info('Order status is not PAID, status: ' . $request->orderStatus);
