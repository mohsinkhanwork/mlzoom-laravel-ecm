    ...
    public function import(Request $request): RedirectResponse
    {
        ...
        foreach ($collections as $collection) {
            ...
            $validator = Validator::make($collection, [
                ...
                'identity_type' => 'required|string|in:passport,driving_license,nid,trade_license,company_id',
                'identity_number' => 'required|string|max:255',
                'latitude' => 'required|numeric',
                'longitude' => 'required|numeric',
                'zone_id' => 'required|uuid',
                'account_email' => 'required|email|max:255|unique:users,email',
                'account_phone' => 'required|string|max:20|unique:users,phone',
            ...
            if (User::where('email', $collection['account_email'])->exists()) {
                Toastr::error(translate('Email already taken'));
                return back();
            }

            if (User::where('phone', $collection['account_phone'])->exists()) {
                Toastr::error(translate('Phone already taken'));
                return back();
            }

            // Process identity images if they exist
            $identityImages = [];
            if (isset($collection['identity_images']) && is_array($collection['identity_images'])) {
                foreach ($collection['identity_images'] as $image) {
                    $identityImages[] = file_uploader('provider/identity/', 'png', $image);
                }
            }
            ...
            try {
                // Create user
                $user = new User();
                $user->email = $collection['account_email'];
                $user->phone = $collection['account_phone'];
                ...
                $provider->company_name = $collection['company_name'];
                $provider->company_phone = $collection['company_phone'];
                $provider->company_email = $collection['company_email'];
                $provider->logo = file_uploader('provider/logo/', 'png', $collection['logo']);
                $provider->company_address = $collection['company_address'];
                $provider->contact_person_name = $collection['contact_person_name'];
                $provider->contact_person_phone = $collection['contact_person_phone'];
                $provider->contact_person_email = $collection['contact_person_email'];
                $provider->is_approved = 1;
                $provider->is_active = 1;
                $provider->zone_id = $collection['zone_id'];
                $provider->coordinates = ['latitude' => $collection['latitude'], 'longitude' => $collection['longitude']];
                $provider->save();

                // Save identity images
                $provider->identification_image()->createMany($identityImages);

                DB::commit();
            } catch (\Exception $e) {
                DB::rollBack();
                Toastr::error(translate('Failed to import data. Please try again later.'));
                return back();
            }
        }
