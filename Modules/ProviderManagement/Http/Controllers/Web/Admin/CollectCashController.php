...
class CollectCashController extends Controller
{
    ...
    public function index($provider_id, Request $request)
    {
        ...
        $total_collected_cash = $this->transaction
           ...
           $provider = $this->provider->with('owner.account')->withCount(['bookings'])->find($provider_id);

        return view('providermanagement::admin.account.collect-cash', compact('transactions','provider', 'total_collected_cash', 'webPage', 'search', 'provider_id'));
    }
}
