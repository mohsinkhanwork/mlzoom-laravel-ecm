    ...
    public function translate(Request $request, $lang): Factory|View|Application
    {
      ...
      if (!empty($searchTerm)) {
            $fullData = array_filter($fullData, function ($value, $key) use ($searchTerm) {
                return (is_string($value) && stripos($value, $searchTerm) !== false) ||
                (is_string($keyTransformed = ucfirst(str_replace('_', ' ', remove_invalid_charcaters($key)))) &&
                 stripos($keyTransformed, $searchTerm) !== false);
